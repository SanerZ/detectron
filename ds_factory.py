...
class fcn_gt(imdb):
    def __init__(self, name, gt_lst, data_path='', image_set=''):
        imdb.__init__(self, name, data_path, image_set)
        self._gt_lst = gt_lst
...
class fcn_lmk(imdb):
    def __init__(self, name, lmk_lst, data_path='', image_set=''):
        imdb.__init__(self, name, data_path, image_set)
...
#                im, *gt = l.strip().split()
                ls = l.strip().split()
                im, gt_lmk = ls[0], ls[1:]
                assert len(gt_lmk)%(self.anno_len * 2) == 0, \
...
class fddb_gt(imdb):
    ...
    def _load_fddb_gt(self):
        ...
        while i < ls:
            ...
            if ngt == 0 or self._image_set and im not in self._image_index:
                continue
