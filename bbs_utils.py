# -*- coding: utf-8 -*-
"""
Created on Mon Aug 20 17:55:18 2018
...
def calc_overlaps(bs1, bs2, wh=False):
    ...
    ovs = np.row_stack(ovs)
    return ovs

def bbox_overlap(bs, b):
    x1 = np.maximum(bs[:, 0], b[0])
    y1 = np.maximum(bs[:, 1], b[1])
    x2 = np.minimum(bs[:, 2], b[2])
    y2 = np.minimum(bs[:, 3], b[3])
    
    w = np.maximum(x2 - x1 + 1., 0.)
    h = np.maximum(y2 - y1 + 1., 0.)
    inter = w * h
 
    uni = (bs[:,2] - bs[:,0] + 1.) * (bs[:,3] - bs[:,1] + 1.) \
